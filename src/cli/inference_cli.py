...
def get_input(
    prompt: str, type_func: Callable[[str], T] = str, valid_options: Optional[List[T]] = None
) -> T:
...
def collect_user_data() -> pd.DataFrame:
    ...
    return pd.DataFrame(
        [{"Level": level, "Location": location, "YearsOfExperience": yoe, "YearsAtCompany": yac}]
    )
...
def main() -> None:
    ...
    if non_interactive:
        ...
        if any([args.level, args.location, args.yoe, args.yac]):
            console.print(
                "[bold red]Error: For non-interactive mode, you must supply --level, --location, --yoe, and --yac.[/bold red]"
            )
            sys.exit(1)
