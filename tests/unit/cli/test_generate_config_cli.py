...
class TestGenerateConfigCLI(unittest.TestCase):
    ...
    def test_heuristic_generation(
        self, mock_print, mock_exists, MockGenerator, mock_read_csv, mock_ears
    ):
        # Setup
    ...
    def test_llm_generation_file_output(
        self, mock_file, mock_exists, MockGenerator, mock_read_csv, MockParser
    ):
        # Setup
        mock_exists.return_value = True
        ...
        # Verify
        mock_gen_instance.generate_config.assert_called_with(
            mock_read_csv.return_value, use_llm=True, provider="openai", preset="salary"
        )
