...
class TestBuildConfigurationPrompt(unittest.TestCase):
    ...
    def test_prompt_with_basic_inputs(self):
        ...
        targets = ["Salary"]
        encodings = {"encodings": {"Level": {"type": "ordinal"}, "Location": {"type": "proximity"}}}
    ...
    def test_prompt_with_correlation_data(self):
        ...
        encodings = {"encodings": {}}
        correlation_data = (
            '{"correlations": [{"column_1": "A", "column_2": "B", "correlation": 0.8}]}'
        )

        prompt = build_configuration_prompt(targets, encodings, correlation_data=correlation_data)
    ...
    def test_prompt_with_column_stats(self):
        ...
        prompt = build_configuration_prompt(targets, encodings, column_stats=column_stats)
    ...
    def test_prompt_with_different_dataset_sizes(self):
        ...
        for size in [10, 100, 1000, 10000]:
            prompt = build_configuration_prompt(targets, encodings, dataset_size=size)
    ...
    def test_prompt_includes_all_context(self):
        ...
        encodings = {
            "encodings": {"Level": {"type": "ordinal", "mapping": {"L1": 0, "L2": 1, "L3": 2}}}
        }
        correlation_data = '{"test": "data"}'
        column_stats = {"Salary": {"mean": 100000}}

        prompt = build_configuration_prompt(targets, encodings, correlation_data, column_stats, 500)
    ...
    def test_prompt_with_ordinal_mapping(self):
        ...
        encodings = {
            "encodings": {
                "Level": {"type": "ordinal", "mapping": {"Junior": 0, "Mid": 1, "Senior": 2}}
            }
        }
...
class TestParseConfigurationResponse(unittest.TestCase):
    ...
    def test_parse_default_hyperparameters_fallback(self):
        ...
        self.assertEqual(result["hyperparameters"]["training"]["objective"], "reg:quantileerror")
...
class TestRunModelConfiguratorSync(unittest.TestCase):
    ...
    def test_successful_configuration(self, mock_load_prompt):
        ...
        result = run_model_configurator_sync(
            mock_llm, ["Salary"], {"encodings": {"Level": {"type": "ordinal"}}}
        )
    ...
    def test_without_tool_calls(self, mock_load_prompt):
        ...
        mock_llm = MagicMock(spec=BaseChatModel)
        mock_response = AIMessage(
            content=json.dumps({"features": [], "quantiles": [0.5], "hyperparameters": {}})
        )

        mock_llm.invoke.return_value = mock_response

        result = run_model_configurator_sync(mock_llm, ["Price"], {"encodings": {}})
    ...
    def test_with_correlation_data(self, mock_load_prompt):
        ...
        result = run_model_configurator_sync(
            mock_llm, ["Price"], {"encodings": {}}, correlation_data=correlation_data
        )
    ...
    def test_with_column_stats(self, mock_load_prompt):
        ...
        column_stats = {"Salary": {"mean": 100000}}
        result = run_model_configurator_sync(
            mock_llm, ["Salary"], {"encodings": {}}, column_stats=column_stats
        )
    ...
    def test_error_handling(self, mock_load_prompt):
        ...
        with self.assertRaises(Exception):
            run_model_configurator_sync(mock_llm, ["Price"], {"encodings": {}})
...
class TestConfigurationValidation(unittest.TestCase):
    ...
    def test_feature_constraints_valid(self):
        ...
        for constraint in valid_constraints:
            response = json.dumps(
                {
                    "features": [{"name": "Test", "monotone_constraint": constraint}],
                    "quantiles": [0.5],
                    "hyperparameters": {},
                }
            )
            ...
            result = parse_configuration_response(response)
            self.assertEqual(result["features"][0]["monotone_constraint"], constraint)

    def test_quantiles_valid_range(self):
        """Test quantiles are in valid range (0-1)."""
        valid_quantiles = [0.0, 0.1, 0.25, 0.5, 0.75, 0.9, 1.0]

        response = json.dumps({"features": [], "quantiles": valid_quantiles, "hyperparameters": {}})
        ...
        response = json.dumps(
            {
                ...
                "hyperparameters": {
                    ...
                    "cv": {"num_boost_round": 200, "nfold": 5, "early_stopping_rounds": 20},
                },
            }
        )
...
class TestModelConfiguratorPreset(unittest.TestCase):
    ...
    def test_preset_none_does_not_load(self, mock_load_prompt):
        ...
        mock_response = AIMessage(
            content=json.dumps(
                {"features": [], "quantiles": [0.5], "hyperparameters": {}, "reasoning": "Test"}
            )
        )
    ...
    def test_preset_invalid_handles_gracefully(self, mock_load_prompt):
        ...
        result = run_model_configurator_sync(
            mock_llm, ["Price"], {"encodings": {}}, preset="invalid_preset"
        )
