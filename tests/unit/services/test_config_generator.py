...
class TestConfigGenerator(unittest.TestCase):
    ...
    def test_infer_features(self):
        data = pd.DataFrame(
            {"Salary": [100000], "YearsOfExperience": [5], "Level": ["L3"], "EmployeeID": [12345]}
        )
    ...
    def test_generate_config_template(self):
        data = pd.DataFrame({"Level": ["L3"], "Location": ["NY"], "Salary": [100000]})
    ...
    def test_generate_config_heuristic_explicit(self):
        data = pd.DataFrame({"Level": ["L3"], "Location": ["NY"], "BaseSalary": [100000]})
